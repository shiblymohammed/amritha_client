...
const RoomCard: React.FC<{ room: Room; onAdd: (roomId: number) => void; onRemove: (roomId: number) => void; quantity: number; isPreSelected?: boolean; }> = ({ room, onAdd, onRemove, quantity, isPreSelected = false }) => (
    ...
    >
        ...
        <div className="relative h-64 overflow-hidden group">
            <LazyImage src={room.image} alt={room.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
        ...
        {[...Array(totalSteps)].map((_, i) => (
            <React.Fragment key={i}>
                <div className="flex flex-col items-center">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${currentStep > i ? 'bg-action-accent text-white' : 'bg-background-tertiary border-2 border-border-soft'}`}>
                        {currentStep > i ? <CheckCircleIcon /> : i + 1}
                    </div>
                </div>
                {i < totalSteps - 1 && <div className={`flex-1 h-1 transition-all duration-300 ${currentStep > i ? 'bg-action-accent' : 'bg-border-soft'}`}></div>}
            </React.Fragment>
        ))}
    </div>
);
        ...
        <div className="bg-background font-cormorant text-text">
            ...
            <main>
                ...
                <section className="relative h-[70vh] overflow-hidden flex items-center justify-center">
                    <div className="absolute inset-0 w-full h-full" style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1561501900-3701fa6a0864?w=1600&h=900&fit=crop)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }} />
                    <div className="absolute inset-0 bg-black/40" />
                    <div className="relative z-10 text-center text-white px-6">
                        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="font-playfair text-4xl md:text-5xl lg:text-6xl mb-6 text-white">Booking</motion.h1>
                        <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="font-cormorant text-lg md:text-xl mb-8 max-w-3xl mx-auto text-white">Select your preferred suite and embark on a journey through time and luxury.</motion.p>
                    </div>
                </section>
                        ...
                        <AnimatePresence mode="wait">
                            {step === 1 && (
                                <motion.div key="step1" variants={stepVariants} initial="hidden" animate="visible" exit="exit">
                                    <h2 className="font-playfair text-h2 text-text-heading mb-8">Choose Your Suite(s)</h2>
                                    {roomsData.map(room => (
                                        <RoomCard key={room.id} room={room} onAdd={handleAddRoom} onRemove={handleRemoveRoom} quantity={selectedRooms[room.id] || 0} isPreSelected={searchParams.get('room') === room.id.toString()} />
                                    ))}
                                    {errors.room && <p className="text-red-500 text-sm mt-4 text-center">{errors.room}</p>}
                                </motion.div>
                            )}
                             {step === 2 && (
                                <motion.div key="step2" variants={stepVariants} initial="hidden" animate="visible" exit="exit">
                                    <h2 className="font-playfair text-h2 text-text-heading mb-8">Guest Information</h2>
                                    <div className="space-y-4 bg-background-secondary p-8 rounded-2xl">
                            ...
                            {step === 3 && (
                                <motion.div key="step3" variants={stepVariants} initial="hidden" animate="visible" exit="exit">
                                    <h2 className="font-playfair text-h2 text-text-heading mb-8">Confirm Your Booking</h2>
                                    <div className="bg-background-secondary p-8 rounded-2xl space-y-4">
                                        <div><strong>Selected Rooms:</strong> {Object.entries(selectedRooms).map(([id, q]) => `${q}x ${roomsData.find(r => r.id === parseInt(id))?.name}`).join(', ')}</div>
                                        <div><strong>Dates:</strong> {bookingDetails.checkIn} to {bookingDetails.checkOut}</div>
                                        <div><strong>Guests:</strong> {bookingDetails.adults} Adults, {bookingDetails.children} Children</div>
                                        <div><strong>Name:</strong> {guestInfo.name}</div>
                                        <div><strong>Email:</strong> {guestInfo.email}</div>
                                        <div><strong>Phone:</strong> {guestInfo.phone}</div>
                                        {guestInfo.requests && <div><strong>Requests:</strong> {guestInfo.requests}</div>}
                                        <div className="font-bold text-lg pt-4 border-t border-border-soft">Total: ₹{priceSummary.total.toLocaleString()}</div>
                                    </div>
                                </motion.div>
                            )}
                        </AnimatePresence>
                        ...
                        <div id="booking-summary" className="bg-background-secondary p-6 rounded-2xl shadow-heritage-lg border border-border-soft">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between mt-4">
                                    {step > 1 && <button onClick={prevStep} className="bg-gray-300 text-gray-800 px-6 py-2 rounded-lg">Previous</button>}
                                    {step < 3 ? <button onClick={nextStep} className="bg-action-accent text-white px-6 py-2 rounded-lg ml-auto">Next</button> :
                                    <button onClick={handleSubmit} disabled={isSubmitting} className="w-full bg-action-accent text-white px-6 py-2 rounded-lg">{isSubmitting ? 'Processing...' : 'Confirm Booking'}</button>}
                                </div>
                            </div>
                        </div>
                    </div>
