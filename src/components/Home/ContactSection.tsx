...
const ContactForm = memo<{ formData: FormData; onInputChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void; onSubmit: () => void }>(({ formData, onInputChange, onSubmit }) => (
  <div className="group animate-fade-in-up">
    <div className="card-base relative h-[600px] p-8 flex flex-col justify-center hover:shadow-golden-glow-sm animate-float">
        ...
        <div className="space-y-6">
          <div className="transform transition-all duration-500 hover:translate-x-2 animate-fade-in">
            <input
              ...
              className="w-full px-6 py-4 bg-background-secondary border border-border text-foreground placeholder-foreground-subtle rounded-xl focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all duration-300 hover:bg-background-tertiary hover:border-accent/50 shadow-soft-sunlight"
          ...
          <div className="transform transition-all duration-500 hover:translate-x-2 animate-fade-in">
            <textarea
              ...
              onChange={onInputChange}
              className="w-full px-6 py-4 bg-background-secondary border border-border text-foreground placeholder-foreground-subtle rounded-xl focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all duration-300 hover:bg-background-tertiary hover:border-accent/50 resize-none shadow-soft-sunlight"
          ...
          <div className="transform transition-all duration-500 hover:scale-105 animate-fade-in">
            <button
              type="button"
              onClick={onSubmit}
              className="btn btn-primary w-full py-4 px-8 text-lg font-semibold shadow-soft-sunlight-lg hover:shadow-golden-glow animate-float transform hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent active:scale-95 transition-all duration-300"
            >
              ✨ Send Heritage Message
            </button>
          </div>
        </div>
      </div>
...
const MapSection = memo(() => (
  <div className="group animate-fade-in-up">
    <div className="card-base relative h-[600px] overflow-hidden img-overlay hover:shadow-golden-glow-sm animate-float">
      {/* Map content */}
